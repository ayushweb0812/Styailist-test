...
export function Nav() {
  ...
  const border = useTransform(scrollY, [0, 80], ["oklch(0.92 0.014 320 / 0)", "oklch(0.92 0.014 320 / 1)"]);
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <a href="/" className="flex items-center gap-2.5 group">
          ...
          <span className="font-display text-xl font-semibold tracking-tight">Styailist<span className="text-primary">.</span></span>
        </a>
        <nav className="hidden md:flex items-center gap-1 text-sm font-medium">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="px-3 py-1.5 rounded-full text-foreground/70 hover:text-ink hover:bg-glass transition-colors">
              {l.label}
            </a>
          ))}
        </nav>
        <div className="flex items-center gap-3">
          <a href="https://my.styailist.com/signup" className="hidden sm:inline text-sm font-medium text-foreground/70 hover:text-ink transition-colors">Sign Up</a>
          <LeadModal>
            <button
